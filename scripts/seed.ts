...
const SAMPLE_PRESS_RELEASES = [
  ...
  },
  {
    ...
    source_url: "https://www.suncheon.go.kr/kr/news/0006/0001/?mode=view&seq=77777",
    published_date: new Date().toISOString(),
  },
];
...
async function main() {
  ...
  const { error: prErr } = await supabase.from("press_releases").insert(
  ...
  console.log(`  Press releases: ${SAMPLE_PRESS_RELEASES.length} inserted`);

  const { error: artErr } = await supabase.from("articles").insert(SAMPLE_ARTICLES);
  ...
  console.log(`  Articles: ${SAMPLE_ARTICLES.length} inserted`);
