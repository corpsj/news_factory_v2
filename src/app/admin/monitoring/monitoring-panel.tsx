        ...
        {SITE_GROUPS.map((group) => {
          ...
          return (
            <div key={group.label}>
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="border-b border-white/[0.06]">
                      <th className="px-5 py-3.5 text-[11px] uppercase tracking-wider text-white/30 font-medium">사이트</th>
                      <th className="px-5 py-3.5 text-[11px] uppercase tracking-wider text-white/30 font-medium">상태</th>
                      <th className="px-5 py-3.5 text-[11px] uppercase tracking-wider text-white/30 font-medium">마지막 수집</th>
                      <th className="px-5 py-3.5 text-[11px] uppercase tracking-wider text-white/30 font-medium">수집 건수</th>
                      <th className="px-5 py-3.5 text-[11px] uppercase tracking-wider text-white/30 font-medium">소요 시간</th>
                      <th className="px-5 py-3.5 text-[11px] uppercase tracking-wider text-white/30 font-medium">연속 실패</th>
                    </tr>
                  </thead>
                  <tbody>
                    {groupSites.map((site) => {
                      ...
                      return (
                        <tr key={site.site_id} className="border-b border-white/[0.04] transition-colors hover:bg-white/[0.03]">
                          ...
                          <td className="px-5 py-3.5 text-white/50 text-xs">
                            {site.last_crawl_at ? formatRelativeTime(site.last_crawl_at) : "기록 없음"}
                          </td>
                          <td className="px-5 py-3.5 text-white/50 text-xs">
                            {site.articles_found}건
                            {site.articles_new > 0 && (
                              <span className="ml-1 text-emerald-400">(+{site.articles_new})</span>
                            )}
                          </td>
                          <td className="px-5 py-3.5 text-white/50 text-xs">
                            {site.crawl_duration_ms !== null ? formatDuration(site.crawl_duration_ms) : "—"}
                          </td>
                          <td className="px-5 py-3.5 text-xs">
                            {site.consecutive_failures > 0 ? (
                              <span className="text-red-400 font-medium">{site.consecutive_failures}회</span>
                            ) : (
              ...
              {groupSites.some((s) => s.failure_reason) && (
                <div className="mt-3 space-y-2">
                  {groupSites.filter((s) => s.failure_reason).map((site) => (
                    <div key={`err-${site.site_id}`} className="rounded-lg border border-red-500/20 bg-red-500/[0.07] px-3 py-2">
                      <p className="text-xs text-red-400">
                      ...
                      {site.error_message && (
                        <p className="mt-0.5 truncate text-[10px] text-red-400/70">{site.error_message}</p>
                      )}
                    </div>
                  ))}
                </div>
